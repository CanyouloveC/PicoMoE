...
static void print_model_runtime(FILE *fp, const help_colors *c,
    ...
    opt(fp, c, "--ssd-streaming", "Metal/CUDA/CPU: opt in to SSD-backed model streaming instead of full residency.");
    opt(fp, c, "--ssd-streaming-cold", "SSD streaming: skip default popularity-based expert-cache preload.");
    opt(fp, c, "--ssd-streaming-cache-experts N|NGB", "SSD streaming: routed expert cache as expert count or GiB, e.g. 32GB. Metal/ROCm default: 80% working set minus non-routed weights; CUDA default: backend fixed cache; CPU default: auto from MemAvailable.");
    ...
    opt(fp, c, "--cpu-usage-file PATH", "CPU streaming: path to per-expert usage stats file (.ds4_usage). Default: <model>.ds4_usage.");
