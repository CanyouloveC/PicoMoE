...
static cli_config parse_options(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        } else if (!strcmp(arg, "--cpu-usage-file")) {
            /* Set via environment for now — the CPU stream reads DS4_CPU_USAGE_FILE */
            setenv("DS4_CPU_USAGE_FILE", need_arg(&i, argc, argv, arg), 1);
